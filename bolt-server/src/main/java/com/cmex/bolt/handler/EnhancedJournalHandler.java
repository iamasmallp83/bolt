...
/**
 * 增强的JournalHandler - 支持从节点确认机制
 * 在从节点模式下，处理完journal写入后向主节点发送确认
 */
@Slf4j
public class EnhancedJournalHandler implements EventHandler<NexusWrapper>, LifecycleAware {
    ...
    public EnhancedJournalHandler(BoltConfig config, TcpSlaveClient tcpSlaveClient) {
        this.config = config;
        ...
        try {
            ...
            log.info("EnhancedJournalHandler initialized with file: {}, isBinary: {}, isMaster: {}",
                    config.journalFilePath(), config.isBinary(), config.isMaster());
        } catch (IOException e) {
    ...
    public void onEvent(NexusWrapper wrapper, long sequence, boolean endOfBatch) throws Exception {
        try {
            ...
            if (!config.enableJournal()) {
                handleSlaveConfirmation(sequence, endOfBatch);
     ...
     * 处理从节点确认机制
     */
    private void handleSlaveConfirmation(long sequence, boolean endOfBatch) {
        if (config.isMaster() || tcpSlaveClient == null) {
            return;
        }
        
        // 更新最后确认的序列号
        lastConfirmedSequence.set(sequence);
        
        // 如果是批次结束，发送确认给主节点
        if (endOfBatch) {
            try {
                long batchId = getCurrentBatchId(sequence);
                tcpSlaveClient.sendConfirmation(batchId, sequence);
                log.debug("Sent confirmation to master for batch {} sequence {}", batchId, sequence);
            } catch (Exception e) {
                log.error("Failed to send confirmation to master for sequence {}", sequence, e);
            }
        }
    }
     ...
     * 简化实现：使用时间戳作为批次ID
     */
    private long getCurrentBatchId(long sequence) {
        // 实际实现中应该从主节点获取真实的批次ID
        // 这里使用简化的实现
        return System.currentTimeMillis() / 1000; // 使用秒级时间戳
    }
     ...
     * 获取最后确认的序列号
     */
    public long getLastConfirmedSequence() {
        return lastConfirmedSequence.get();
    }
    ...
    public void setTcpSlaveClient(TcpSlaveClient tcpSlaveClient) {
        this.tcpSlaveClient = tcpSlaveClient;
        log.info("TcpSlaveClient reference set in EnhancedJournalHandler");
    }

    @Override
    public void onStart() {
        final Thread currentThread = Thread.currentThread();
        currentThread.setName(EnhancedJournalHandler.class.getSimpleName() + "-thread");
        log.info("EnhancedJournalHandler started - isMaster: {}", config.isMaster());
    }
    ...
    public void onShutdown() {
        try {
            ...
            log.info("EnhancedJournalHandler shutdown completed");
        } catch (IOException e) {
            log.error("Error during EnhancedJournalHandler shutdown", e);
        }
    }
}
