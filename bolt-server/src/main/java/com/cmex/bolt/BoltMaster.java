...
public class BoltMaster extends BoltBase {
    ...
    public BoltMaster(BoltConfig config) {
        ...
        log.info("BoltMaster initialized - replication enabled: {}, replication port: {}", 
                config.enableReplication(), config.replicationPort());
    }
    ...
    protected void startNodeSpecificServices() {
        ...
        if (config.enableReplication()) {
            ...
            this.replicationServer = new TcpReplicationServer(config.replicationPort(), config);
            
            try {
                replicationServer.start();
                log.info("Replication server started successfully on port {}", config.replicationPort());
            ...
            log.info("Replication server not started - replication disabled");
        }
    }
