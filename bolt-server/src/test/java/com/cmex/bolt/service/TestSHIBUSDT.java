...
public class TestSHIBUSDT {
    ...
    public void testShibUsdt() throws InterruptedException {
        ...
        EnvoyUtil.placeOrder(service, 1, 2, 4, Envoy.Type.LIMIT, Envoy.Side.ASK, "0.00000860", "10000000",
                FakeStreamObserver.of(response -> {
                    Assertions.assertTrue(response.getData().getId() > 0);
                    latch.countDown();
                }));
        EnvoyUtil.placeOrder(service, 1, 2, 3, Envoy.Type.LIMIT, Envoy.Side.BID, "0.00000860", "10000000",
                FakeStreamObserver.of(response -> {
                    Assertions.assertTrue(response.getData().getId() > 0);
                    latch.countDown();
                }));
    ...
    public void testShibUsdt1() throws InterruptedException {
        ...
        latch.await();
        Thread.sleep(1000);
        EnvoyUtil.getAccount(service, 5, FakeStreamObserver.of(response -> {
        ...
        EnvoyUtil.getAccount(service, 6, FakeStreamObserver.of(response -> {
